...
interface EnterIncomeProps {
  enterIncomeActive: boolean;
  setEnterIncomeActive: Dispatch<SetStateAction<boolean>>;
  success: boolean;
  setSuccess: Dispatch<SetStateAction<boolean>>;
}

const EnterIncome: FC<EnterIncomeProps> = ({
  enterIncomeActive,
  setEnterIncomeActive,
  success,
  setSuccess,
}) => {
  ...
  const onSubmit = async () => {
    ...
    if (income.length === 0 || date === "") {
      setEmptyError(true);
      setLoading(false);
    } else {
      try {
        const docRef = doc(db, "income", date);
          ...
          {error === true && (
            <div className="flex items-center justify-center h-full -mt-4 w-full">
              <p className="text-sm text-center text-red-600 mt-4">
                An error occured, could not add income
              </p>
            </div>
          )}
